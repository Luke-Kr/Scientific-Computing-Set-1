...
def time_stepping(u, c, dt, dx, initial_condition, num_steps, boundary='dirichlet'):
    ...
    for j in range(1, N):
        u[j, 1] = u[j, 0] + 0.5 * \
            ((c * dt / dx) ** 2) * (u[j + 1, 0] - 2 * u[j, 0] + u[j - 1, 0])
    ...
    for step in range(2, num_steps):
        for j in range(1, N):
            u[j, 2] = (2 * u[j, 1] - u[j, 0] + (c * dt / dx) **
                       2 * (u[j + 1, 1] - 2 * u[j, 1] + u[j - 1, 1]))
...
def plot_wave_states(results, x, num_time_points=3):
    ...
    time_indices = np.linspace(
        0, results.shape[2] - 1, num_time_points, dtype=int)
    ...
    for i in range(num_conditions):
        ...
        axs[i].set_title(
            f'Wave Evolution for Initial Condition {i + 1}', fontsize=26)
...
def animate_wave(results, x, init_condition_index, filename):
    ...
    ani = animation.FuncAnimation(
        fig, update, frames=results.shape[2], blit=True, interval=1)
